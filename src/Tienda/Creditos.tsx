...
const Creditos = () => {
    ...
    const consultarCreditos = useCallback(() => {
        httpGet<ICredito[]>(`${urlBase}creditos/${tienda.id}`, { headers: { Authorization: sesion.sesion.token } })
            .then(result => {
    ...
    useEffect(() => {
        consultarCreditos()
    }, [tienda.id, sesion.sesion.token, consultarCreditos])

    const redirectCredito = (credito: number) => {
        dispacher(setCredito(credito))
    ...
    const consultarCredito = (event: ChangeEvent<any>) => {
        ...
        // dispache(setSpiner(true))
        if (event.target.value) {
            httpGet<ICredito[]>(`http://localhost:8080/api/creditos/cedula/${event.target.value}`, { headers: { Authorization: sesion.sesion.token } })
                .then(result => {
